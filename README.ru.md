...
<p align="center">
  <a href="https://github.com/neket371/network-stealth-core/releases"><img alt="release" src="https://img.shields.io/badge/release-v7.10.7-0f766e"></a>
...
```bash
curl -fL https://raw.githubusercontent.com/neket371/network-stealth-core/v7.10.7/xray-reality.sh -o /tmp/xray-reality.sh
sudo XRAY_REPO_REF=v7.10.7 bash /tmp/xray-reality.sh install
```
...
- `XRAY_DATA_DIR` в wrapper-режиме не является свободным trusted source; `XRAY_ALLOW_CUSTOM_DATA_DIR=true` используй только для trusted non-world-writable source tree, где sourced shell-файлы, runtime-assets и symlink-target'ы остаются внутри этого дерева
...
- direct или сохранённый trusted `XRAY_DATA_DIR` теперь остаётся приоритетным даже рядом со stale локальным bootstrap tree, а wrapper/bootstrap loading держится на одном coherent managed source tree вместо тихой склейки двух разных деревьев
