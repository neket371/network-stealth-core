...
<p align="center">
  <a href="https://github.com/neket371/network-stealth-core/releases"><img alt="release" src="https://img.shields.io/badge/release-v7.10.6-0f766e"></a>
...
```bash
curl -fL https://raw.githubusercontent.com/neket371/network-stealth-core/v7.10.6/xray-reality.sh -o /tmp/xray-reality.sh
sudo XRAY_REPO_REF=v7.10.6 bash /tmp/xray-reality.sh install
```
...
| `doctor` | короткий operator verdict и следующее действие |
...
- `update --replan` и `repair` теперь используют один cooldown-aware rotation engine, поэтому слабый primary и недавно деградировавшие provider family не возвращаются в приоритет сразу
- `doctor` — read-only одностраничный verdict по runtime, последнему self-check, rotation state, cooldown reason и следующему действию
- `status --verbose`, `doctor`, `diagnose`, `repair`, `update --replan` и `scripts/measure-stealth.sh summarize` теперь читают один общий operator decision layer: verdict, качество покрытия, diversity по provider family, long-term trend, rotation verdict, cooldown state, рекомендацию и promotion candidate
...
- `/var/lib/xray/measurements/latest-summary.json` — агрегированный field verdict для `status --verbose`, `doctor`, `diagnose`, `repair` и `update --replan`
- `/var/lib/xray/measurements/rotation-state.json` — сохранённое состояние weak-primary streak, cooldown family/domain и последнего promotion context
...
`summarize` теперь печатает тот же decision-grade слой, на который потом опираются `status --verbose`, `doctor`, `diagnose`, `repair` и `update --replan`: качество покрытия, spread по сетям и провайдерам, статистику current primary, статистику лучшего spare, rotation verdict, cooldown state и итоговую рекомендацию.
...
- `XRAY_DATA_DIR` в wrapper-режиме не является свободным trusted source; `XRAY_ALLOW_CUSTOM_DATA_DIR=true` используй только для trusted non-world-writable source tree, где sourced shell-файлы и symlink-target'ы остаются внутри этого дерева
- managed self-sync wrapper’а зеркалит runnable managed source set внутри `XRAY_DATA_DIR`, а не весь checkout репозитория; stale-файлы, уже удалённые из этого managed source set, при следующем sync исчезают, а не переживают апдейт случайно
- сохранённые `XRAY_DATA_DIR` и `XRAY_ALLOW_CUSTOM_DATA_DIR` теперь переживают обычный `config.env` round-trip, так что trusted custom managed source tree на следующем запуске wrapper’а остаётся приоритетным и не откатывается молча ни к stale локальному tree, ни к default tree
- custom `XRAY_GEOIP_URL` / `XRAY_GEOSITE_URL` и их checksum URL теперь должны оставаться на хостах из `DOWNLOAD_HOST_ALLOWLIST`; geo refresh идёт через обычный update-path и сохраняет предыдущие рабочие assets, если checksum verification падает
