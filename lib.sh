#!/usr/bin/env bash
# Network Stealth Core 7.10.4 - Автоматизация strongest-direct Xray Reality (policy, schema v3, canary, adaptive repair, doctor)

set -euo pipefail

SCRIPT_DIR="${SCRIPT_DIR:-$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)}"

readonly SCRIPT_VERSION="7.10.4"
...
XRAY_SOURCE_KIND="${XRAY_SOURCE_KIND:-}"
XRAY_SOURCE_REF="${XRAY_SOURCE_REF:-}"
...
main() {
    ...
    case "$ACTION" in
        ...
        diagnose)
            ...
            ;;
        doctor)
            strict_validate_runtime_inputs "doctor"
            doctor_flow
            ;;
        rollback)
